...
<body>
	...
	<div class="mdl-grid" align="center">
			...
			<form method="post">
				<div>
					<span class="mdl-typography--title mdl-color-text--grey-700">
						Enter equipment option to fetch cars:
					</span>
				</div>
				<div class="mdl-textfield mdl-js-textfield" style="width:150px">
					<input autofocus class = "mdl-textfield__input" type="text" id="opt" name="opt" pattern=".{1,30}"/>
					<label class = "mdl-textfield__label mdl-color-text--grey-600" for = "opt">Option</label>
					<span class="mdl-textfield__error">Max 30 chars</span>
				</div>
				<div style="width:100%">
					<button type="submit" value="fetchCar"
							class="mdl-button mdl-color--red-900 mdl-js-button mdl-color-text--grey-100
									mdl-js-ripple-effect mdl-button--raised" name="fetchCar">
					Find
					</button>
				</div>
			</form>
		...
		<%
		if(request.getParameter("fetchCar") != null){
			String equipOpt = request.getParameter("opt").toUpperCase();
			ConnectionManager connMgr = new ConnectionManager();
			ResultSet rs = connMgr.getCarByEquipOpt(uid, pwd, equipOpt);
			if (rs.isBeforeFirst()) {%>
	<div class="mdl-cell mdl-cell--12-col" style="margin: 1em">
		<br />
		<div class="mdl-typography--title mdl-color-text--red-900">CAR DETAILS</div>
		<br />
