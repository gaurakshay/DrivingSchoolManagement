...
<body>
	...
	<div class="mdl-grid" align="center">
			...
			<form method="post">
				<div>
					<span class="mdl-typography--title mdl-color-text--grey-700">
						Enter weight(lbs) to find truck owners:
					</span>
				</div>
				<div class="mdl-textfield mdl-js-textfield" style="width:150px">
					<input autofocus class = "mdl-textfield__input" type="text" id="wght" name="wght" pattern="[0-9]{1,5}"/>
					<label class = "mdl-textfield__label mdl-color-text--grey-600" for = "wght">Weight</label>
					<span class="mdl-textfield__error">Max 5 digits</span>
				</div>
				<div style="width:100%">
					<button type="submit" value="fetchInst"
							class="mdl-button mdl-color--red-900 mdl-js-button mdl-color-text--grey-100
									mdl-js-ripple-effect mdl-button--raised" name="fetchInst">
					Find
					</button>
				</div>
			</form>
		...
		<%
		if(request.getParameter("fetchInst") != null){
			String wght = request.getParameter("wght");
			ConnectionManager connMgr = new ConnectionManager();
			ResultSet rs = connMgr.getInstByWght(uid, pwd, wght);
			if (rs.isBeforeFirst()) {%>
	<div class="mdl-cell mdl-cell--12-col" style="margin: 1em">
		<br />
		<div class="mdl-typography--title mdl-color-text--red-900">INSTRUCTOR DETAILS</div>
		<br />
